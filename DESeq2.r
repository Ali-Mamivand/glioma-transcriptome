##---- loading packages --------------
# important Note: use the last version of TCGAbiolinks and TCGAbiolinksGUI.data (https://github.com/BioinformaticsFMRP/TCGAbiolinks)
...
sample_sheet <- read.delim("C:/path/to/working/directory/sample_sheet.tsv")
query_exp <- GDCquery(project= c("TCGA-LGG","TCGA-GBM"), 
                      data.category = "Transcriptome Profiling", 
                      workflow.type = 'STAR - Counts',
                      data.type = 'Gene Expression Quantification',
                      barcode = sample_sheet$Sample.ID)
...
rownames(res)= str_sub(rownames(res),1,15)
...
# volcano plot. The following function creates a PDF file (volcano.pdf) in the working directory:
TCGAVisualize_volcano(x = res$log2FoldChange,y = res$padj,x.cut = c(-2,2),
                      y.cut = 0.001,xlab = "Log2 (Fold Change)",height = 6,
                      width = 8.5,names = res$symbol,highlight = 'OBI1-AS1',
                      show.names='highlighted',
                      label = c('Not Significant','Up-Regulated in LGG',
                                'Down-Regulated in LGG'))
##---- report results--------
...
pathway_Dir = 'F:/your/directory/' #set it to the your directory that contains gmt file
